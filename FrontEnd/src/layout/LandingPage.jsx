...
const LandingPage = () => {
  ...
  const handleAddMenu = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:3100/pembeli/create",
        dataAdd
      );
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:3100/pembeli/login",
        dataAdd
      );
      console.log("Login berhasil:", response.data.data);
      localStorage.setItem("dataku", JSON.stringify(response.data.data));
  ...
  const handleAddCart = async () => { 
    ...
    console.log(idStock);
    try {
      const response = await axios.post("http://localhost:3100/cart/create", {
        ...dataTr,
        BuahId: idMenu,
      });
      console.log("Login berhasil:", response.data.data);
  ...
  async function handlePesan() {
    ...
    try {
      const response = await axios.post(
        "http://localhost:3100/transaksi/create",
        {
          total_harga: totalHarga,
          status: true,
          tanggal_pembelian: new Date(),
          CartId: idCart,
          PembeliId: parseId.id,
        }
      );
      console.log("Login berhasil:", response.data.data);
      ...
      try {
        ...
        const responseBuah = await axios.put(
          "http://localhost:3100/menu/update/" + dataB.id,
          { ...dataB, stock: dataB.stock - dataTr.jumlah_barang }
        );
        window.location.href = "/";
      } catch (err) {
  ...
  return (
    <div>
      ...
      <div className="max-w-6xl mx-auto mt-20 ">
        ...
        <ModalPembeli
          judul=" Masuk ke Data yang terdaftar"
          data={dataAdd}
          onSub={handleLogin}
          setData={setDataAdd}
          state={{ modalAdd: modalLog, setModalAdd: setModalLog }}
        ></ModalPembeli>
        ...
        ></ModalAddTran>
        {modalAprove ? <Popup onSub={handlePesan}></Popup> : <></>}
        <div className="w-full flex flex-wrap gap-8">
          {dataMenu.map((e, i) => (
            <div
              key={i}
              className="relative flex flex-col  bg-white shadow-sm border border-slate-200 rounded-lg w-65"
            >
              ...
              <div className="px-4 pb-4 w-full flex items-end justify-between  gap-2">
                <p className="font-semibold">{convertToRp(e.harga)}</p>
